...
class TestOptions:
	...
	def initialize(self):
		...
		## arguments for extracting latent vectors of encoder ##
		self.parser.add_argument('--save_latents', action='store_true', help='Whether to save output latent vectors of encoder')

		## arguments for latent operations
		self.parser.add_argument('--do_latent_ops', action='store_true', help='Whether to calculate latent vectors by referring to confident latent vectors')
		
		### bayesian inference(?)
		self.parser.add_argument('--bayesian', action='store_true', help='bayesian')
